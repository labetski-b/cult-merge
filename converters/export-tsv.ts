...
import krakenQuestSequence from '../src/data/kraken_quest_sequence.json';
...
function buildMergeNests(): string {
  ...
  function computeSpawnRow(
    ...
    return [
      String(levelIdx + 1), price, amount, duration, mergesRequired, id,
  ...
  for (const genId of genOrder) {
    ...
    for (let i = 0; i < gen.levels.length; i++) {
      ...
      let amount = '';
      if (i === 0) {
        price = mapCurrency(gen.purchaseCurrency);
        amount = String(gen.purchaseCost);
      } else {
        const prev = gen.levels[i - 1]!;
        const dur = prev.upgrade?.upgradeDurationSec;
        duration = typeof dur === 'number' ? fmt(dur) : '';
        const mr = prev.upgrade?.spawnsRequired;
        mergesRequired = typeof mr === 'number' ? String(mr) : '';
        if (prev.upgrade) {
          price = mapCurrency(prev.upgrade.runeType);
          amount = String(prev.upgrade.runeCost);
        }
      }
      rows.push(computeSpawnRow(
        lvl.outputs, gen.lines as [string, string],
        numCreatures, chargeCost,
        i, price, amount, idValue,
        duration, mergesRequired,
      ));
    }
  }
...
function buildMergeQuestsInfo(): string {
  ...
  for (const chapter of krakenQuestSequence.chapters) {
    for (let index = 0; index < chapter.subquests.length; index++) {
      const subquest = chapter.subquests[index]!;
      rows.push([
        index === 0 ? String(chapter.questId) : '',
        String(subquest.subQuestId),
        subquest.type,
        subquest.value,
        String(subquest.amount),
        subquest.reward,
        String(subquest.rewardAmount),
        index === 0 ? String(chapter.krakenLevel) : '',
      ]);
    }
  }
