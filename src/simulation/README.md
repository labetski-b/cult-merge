...
Все графики-срезы, кроме **Per Task**, используют `result.actionHistory`: один полный snapshot на каждый залогированный action/event.
Это важно, потому что один outer tick симулятора может выполнить много действий и пройти короткий Kraken level; обычный `result.history` хранит только end-of-tick snapshots и для аналитики по срезам не является точным.
...
- **`spawnsSpentByGenSnapshot`** — копия legacy-поля `state.spawnsSpentByGen`
...
- **Прокачка генераторов (countdown timed-process slot)** — после Task 3 plan
  ...
  - **Spawn-gate**: количество спавнов из самого генератора
    (`state.spawnCountByGen[genId]`) должно достичь `spawnsRequired` нужного
    уровня, иначе `start_upgrade` отклоняется. После завершения и сбора
    апгрейда `spawnCountByGen[genId]` сбрасывается в 0, поэтому следующий
    `spawnsRequired` означает количество спавнов после предыдущего апгрейда.
