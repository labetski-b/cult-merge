...
function getAutoQuestScoringRuntimeConfig(): {
...
export function getExpectedL1PerCharge(
  ...
  // Timer-mode generators don't have a "per charge" notion. Returning 0 here
  // lets the mandatory-task reward lookup use its explicit timer fallback.
  if (levelConfig.mode !== 'sacrifice') return 0;

  let total = 0;
  for (const output of levelConfig.outputs) {
    if (output.creatureType === creatureType) {
      total += getGeneratorOutputChance(output)
        * levelConfig.numCreatures
        * Math.pow(2, output.level - 1);
    }
  }
  return total;
}
 ...
 * Flower Pot scoring:
 * `spawnL1 = FP_EXPECTED_SPAWNS × Σ slotChance × chance × 2^(L-1)`.
 */
const FP_EXPECTED_SPAWNS = 8;

/**
 * Build the compact generator-efficiency lookup used only for mandatory-task
 * reward stamping. Auto-task generation uses `buildAutoQuestScoringTable`.
 */
...
function buildScoringTable(
  ...
  for (const candidate of uniqueCandidates) {
    ...
    for (const ct of types) {
      ...
      // multiplier), so the per-spawn L1 for `ct` is
      // `Σ_{ct} slotChance × chance × 2^(L-1)`.
      // Sacrifice gens use the shared per-charge formula in `getExpectedL1PerCharge`.
      let l1pc: number;
      ...
      if (levelConfig.mode === 'timer') {
        ...
        // Keep `l1pc` populated with the per-spawn yield so mandatory-task
        // reward stamping has a non-zero timer value.
 ...
 *
 * Used by `getActiveTask` for mandatory quests, which are stamped on read so
 * chapter + generator economics come from current game state rather than the
 * JSON definition. Auto quests use `computeEyeRewardFromScoringRows`.
 */
...
/**
 * Build the `l1PerMeat` lookup for mandatory-quest eye-reward stamping.
 */
...
export function computeAutoQuestEyeRewardFromScoringRows(
  config: BalanceConfig,
  state: GameSnapshot,
...
function makeTaskFromScoringRows(
  ...
  const reward = computeAutoQuestEyeRewardFromScoringRows(config, state, rows);
...
function generateAutoTaskFromScoringTable(
  config: BalanceConfig,
  state: GameSnapshot,
  rng: SeededRng,
): TaskDefinition {
  const runtimeConfig = getAutoQuestScoringRuntimeConfig();
  ...
  const fallbackReward = computeAutoQuestEyeRewardFromScoringRows(config, state, []);
  return {
    id: makeTaskId(rng),
...
export function generateAutoTask(
  ...
  return generateAutoTaskFromScoringTable(config, state, rng);
}
