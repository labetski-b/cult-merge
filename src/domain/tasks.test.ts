    ...
    levels: [
      {
        ...
        chargeCost: 0.5,
        numCreatures: 15,
        outputs: [{ creatureType: 'Creature1', level: 1, slotChance: 1, chance: 1 }],
        upgrade: {
          spawnsRequired: 15,
    ...
    levels: [
      {
        ...
        chargeCost: 0.5,
        numCreatures: 15,
        outputs: [{ creatureType: 'Creature3', level: 1, slotChance: 1, chance: 1 }],
      },
    ],
  };
...
function getDecisionRows(task: TaskDefinition) {
  const decision = task.debugAutoQuestDecision;
  return [
...
describe('generateAutoTask — scoring table sources', () => {
  it('considers only generators physically on the field (no phantom purchases)', () => {
    ...
    const genIds = new Set(getDecisionRows(task).map((e) => e.genId));
...
describe('generateAutoTask — difficulty 1 cheap quest rerun', () => {
  it('reruns scoring as difficulty 2 when the difficulty 1 pick requires less L1 than 10 generator spawns', () => {
    ...
    expect(task.debugAutoQuestDecision?.contexts.every(
      (context) => context.meatBudget === task.debugMeatBudget,
    )).toBe(true);
  });
});
...
describe('generateAutoTask — phantom +1 upgrade gating', () => {
  it('uses scoringLevel = factLvl + 1 when upgrade is affordable (runes + spawns OK)', () => {
    ...
    const gen1Row = getDecisionRows(task).find((e) => e.genId === 1);
    ...
    levels: [
      {
        ...
        tickIntervalSec: 1800,
        outputs: [
          { creatureType: 'Creature5', level: 1, slotChance: 0.6, chance: 1 },
          { creatureType: 'Creature7', level: 1, slotChance: 0.4, chance: 1 },
        ],
      },
    ],
  };
...
describe('generateAutoTask — Flower Pot scoring', () => {
  it('scores timer-mode generator with 8-tick window formula', () => {
    ...
    state.autoTaskLineCompletions = { Creature1: 8 };

    const task = generateAutoTask(config, state, new SeededRng(1));

    const fpRowC5 = getDecisionRows(task).find(
    ...
    //             = 8 × (0.6 × 2^0) = 4.8
    expect(fpRowC5!.spawnL1Capacity).toBeCloseTo(4.8, 2);
  });

});

describe('generateAutoTask — Flower Pot difficulty windows', () => {
  ...
  it('uses the scoring-table algorithm in Node/Vitest with a compact debug payload', () => {
    const state = makeSnapshotWithGen1OnField();
