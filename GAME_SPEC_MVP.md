...
### 1.3 Авто-квесты (Auto-tasks)

**Статус**: Реализовано в MVP

После выполнения всех mandatory tasks на текущем уровне автоматически генерируются авто-квесты.

**Алгоритм генерации** (`src/domain/tasks.ts` → `generateAutoTask`):

**Step 0 — Высокоуровневое существо на поле (50% шанс)**:
- Ищет существ уровня **6+** на поле (только `kind === 'creature'`)
- Исключает существо, совпадающее с предыдущим квестом (тип + уровень)
- Если есть подходящие и бросок < 0.5 — квест на это существо (count: 1)

**Steps 1-3 — Потенциал генераторов**:
- Считает доступные руны (ресурсы + руны на поле + содержимое коробок)
- Строит потенциальный пул существ из заряженных/покупаемых генераторов
- Симулирует мердж генераторов для расчёта максимального уровня

**Steps 4-5 — Выбор линии существ**:
- 90% шанс исключить линию предыдущего квеста
- Взвешенный выбор линии по старшинству генератора (70/20/7/3%)

**Steps 6-8 — Уровень и количество**:
- Count: 1–4 (случайно)
- Уровень: `[maxLvl-3 .. maxLvl + floor(completions/3)]`, с поправкой на count
- Ограничение: `level ≤ maxLevel ∧ level ≤ fieldLevelCap`

**Anti-duplicate**: До 10 попыток чтобы не повторить предыдущий квест

**Только существа**: Квесты генерируются исключительно на creature-сущности. Коробки, руны, цветочные горшки и хищники не могут быть целью квеста.
...
**Для LVL 8+**: Таблицы mandatory tasks не предоставлены — используются авто-квесты (см. раздел 1.3).

---

### 2.3 Условия появления авто-квестов

**Ответ**: Реализовано. См. раздел 1.3.

- Авто-квест появляется после выполнения всех mandatory tasks на текущем уровне
- Генерация учитывает доступные генераторы, существ на поле и историю квестов
- Целью квеста могут быть только существа (creature), не коробки/руны/цветочные горшки/хищники
...
### 2.6 Правило выдачи мяса

**Ответ**: **Кнопка "Get Meat" с расчётом по главам**

**Механика**:
Большая кнопка "Get Meat" в интерфейсе. При нажатии выдаёт N мяса, рассчитанное на основе текущей главы игрока.

**Определение главы**:
- Глава определяется по суммарному количеству заработанных глаз (Eyes)
- Данные из `chapters_data_analytics.json`: поле `merge_resource` — кол-во глаз, заработанных в каждой главе
- Кумулятивные пороги рассчитываются суммированием:
  - Глава 2: 0–245 глаз (246 глаз в главе)
  - Глава 3: 246–2698 (2453 глаз в главе)
  - Глава 4: 2699–7928 (5230 глаз в главе)
  - и т.д.

**Расчёт мяса (интерполяция)**:
```typescript
progress = (totalEyes - chapterStartEyes) / (chapterEndEyes - chapterStartEyes)
meat = Math.round(meatMin + progress * (meatMax - meatMin))
```
Линейная интерполяция между `meat_min` и `meat_max` текущей главы.

**Система сессий**:
- Счётчик нажатий кнопки. На старте: сессия = 1, нажатий = 0
- Каждые 5 нажатий (на 6-м, 11-м, 16-м и т.д.) номер сессии увеличивается ПЕРЕД начислением мяса
- Формула: `session = pressCount <= 5 ? 1 : 1 + floor((pressCount - 1) / 5)`

**UI**:
- Панель с отображением текущей главы и сессии
- Большая кнопка с превью кол-ва мяса
- Глава также отображается в HeaderBar

**Файл данных**: `src/data/chapters_data_analytics.json`
**Доменная логика**: `src/domain/chapters.ts`
